...
if len(puntos) == 10:
    ...
    # Convertir el set a un array de numpy para facilitar los cálculos
    no_0_255 = np.array(list(no_0_255))

    # Calcular la media y la desviación estándar para cada canal (R, G, B)
    medias = np.mean(no_0_255, axis=0)
    desviaciones = np.std(no_0_255, axis=0)

    # Mostrar los resultados
    print(f"Media de los canales R, G, B: {medias}")
    print(f"Desviación estándar de los canales R, G, B: {desviaciones}")

    # Guardar los resultados en un archivo txt
    with open('estadisticas_rgb.txt', 'w') as f:
        f.write(f"Media de los canales R, G, B: {medias}\n")
        f.write(f"Desviación estándar de los canales R, G, B: {desviaciones}\n")

    print(f"Se han guardado las estadísticas en el archivo 'estadisticas_rgb.txt'.")
